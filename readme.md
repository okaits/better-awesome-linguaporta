...
[LINGUAPORTA](https://w5.linguaporta.jp/user/seibido/index.php)を効率化する拡張機能です。
...
## 機能
### 緊急停止ボタン
    - 画面右上に「▶ 自動進行 ON」ボタンが表示される
    - ボタンをクリックすると「⏸ 自動進行 OFF」に切り替わり、自動進行が停止する
    - 再度クリックすると自動進行が再開される

### Unit「単語の意味」
    - 教材を選択した時点で自動的に「次やるべき選択問題」が始まる
    - 選択肢を選択したら自動的に送信される
    - 正解だったら、自動的に次の問題が表示される
    - キーボードで数字キーを押下した際に自動的に選択肢が選択され、送信される
    - キーボードでone, two, thr(ee), fou(r), fiv(e)と入力した場合、自動的に選択肢が選択され、送信される（ミスタイプ対策）
    - キーボードでone, two, thr(ee), fou(r), fiv(e)のいずれかを同時に押した場合も、自動的に選択肢が選択され、送信される
    - 選択肢それぞれの先頭に番号がつく
...
1. 拡張機能が読み込まれる（この時`エラー`と表示されるかもしれないが、Chromeのバグなので気にしなくてよい。）
1. [LINGUAPORTA](https://w5.linguaporta.jp/user/seibido/index.php)を開くと作動する。
...
3. [LINGUAPORTA](https://w5.linguaporta.jp/user/seibido/index.php)を開くと作動する。
